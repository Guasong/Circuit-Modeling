...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(d_model, configs.c_out),
        )
    ...
    def padding_forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        src = self.lin_input(x_enc)
        last_timestamp = src[:, -1:, :].repeat(1, self.pred_len, 1)
        src = torch.cat((src, last_timestamp), dim=1)

        time_steps_to_predict = torch.linspace(0, 1, src.shape[1]).to(x_enc.device)
        time_steps_to_predict = time_steps_to_predict.unsqueeze(0).repeat(x_enc.shape[0], 1)

        latents = self.encoder(src, time_steps_to_predict)
        outputs = self.decoder(latents)

        dec_out = outputs[:, -self.pred_len:, :]
    ...
    def interpolate_forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        qt = torch.linspace(0, 1, x_enc.shape[1] + self.pred_len).to(x_enc.device)
        qt = qt.unsqueeze(0).repeat(x_enc.shape[0], 1)
        t = qt[:, :x_enc.shape[1]]
        x_enc = self.lin_input(x_enc)

        latents = self.encoder.interpolate(x_enc, t, qt)
        outputs = self.decoder(latents)
